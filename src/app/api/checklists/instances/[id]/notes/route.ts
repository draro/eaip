  ...
  try {
    ...
    if (instance.organization.toString() !== user.organization?.toString() &&
      ...
      return NextResponse.json(
        { error: 'You do not have permission to view notes for this checklist' },
        { status: 403 }
      );
    }
    ...
    // Filter by page number if provided (for checklist sections)
    if (pageNumber) {
...
export async function POST(
  ...
  try {
    ...
    if (instance.organization.toString() !== user.organization?.toString() &&
        user.role !== 'super_admin') {
      return NextResponse.json(
        { error: 'You do not have permission to add notes to this checklist' },
        { status: 403 }
      );
    }
